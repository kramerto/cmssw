...
private:
  ...
  bool useHSCPCalibration_;
  bool useMyCalibration_;
  ...
  unsigned int run_;
  std::vector<unsigned int> APVId_;
  std::vector<unsigned int> ROCId_;
  std::vector<unsigned int> stripRawId_;
  std::vector<unsigned int> pixelRawId_;
  std::vector<unsigned int> pixelSubDetId_;
  std::vector<unsigned int> stripSubDetId_;
  std::vector<double> pixelHitPhi_;
  std::vector<double> stripHitPhi_;
  std::vector<double> pixelHitEta_;
  std::vector<double> stripHitEta_;
};
...
DemoAnalyzer::DemoAnalyzer(const edm::ParameterSet &iConfig)
    : tracks_(iConfig.getUntrackedParameter<std::string>("tracks",
      ...
      useScalefactors_(
          iConfig.getUntrackedParameter<bool>("useScalefactors", false)),
      useHSCPCalibration_(
          iConfig.getUntrackedParameter<bool>("useHSCPCalibration", false)),
      useMyCalibration_(
          iConfig.getUntrackedParameter<bool>("useMyCalibration", false)),
      nFoundHitsCut_(
  ...
  TrackerGains = NULL;
  run_ = 0;
  myCalibration = new std::unordered_map<unsigned int, double>;
  ...
  tree_->Branch("APVId", &APVId_);
  tree_->Branch("ROCId", &ROCId_);
  tree_->Branch("stripRawId", &stripRawId_);
  tree_->Branch("pixelRawId", &pixelRawId_);
  tree_->Branch("pixelSubDetId", &pixelSubDetId_);
  tree_->Branch("stripSubDetId", &stripSubDetId_);
  tree_->Branch("pixelHitPhi", &pixelHitPhi_);
  tree_->Branch("pixelHitEta", &pixelHitEta_);
  tree_->Branch("stripHitPhi", &stripHitPhi_);
  tree_->Branch("stripHitEta", &stripHitEta_);
}
...
void DemoAnalyzer::analyze(const edm::Event &iEvent,
  ...
  stripRawId_.clear();
  pixelRawId_.clear();
  pixelSubDetId_.clear();
  stripSubDetId_.clear();
  pixelHitPhi_.clear();
  pixelHitEta_.clear();
  stripHitPhi_.clear();
  stripHitEta_.clear();
    ...
    if ((std::abs(tracks->at(i).eta()) < etaCut_) &&
      ...
      if (dedxHits) {
        ...
        for (unsigned int h = 0; h < dedxHits->size(); h++) {
          ...
          if (detid.subdetId() >= 3) {
            if (clusterCleaning_ &&
                !clusterCleaning(dedxHits->stripCluster(h), crossTalkInvAlgo_))
              continue;
            ...
            for (unsigned int s = 0; s < amplitudes.size(); s++) {
              ...
              APV = (firstStrip + s) / 128;
              if (APV != prevAPV) {
                if ((TrackerGains != NULL) && (useHSCPCalibration_)) {
                  gain = TrackerGains->at(detid.rawId() << 3 | APV);
                }
                if ((myCalibration != NULL) && (useMyCalibration_)) {
                  ...
                  //                            << ", gain: "
                  //                            <<
                  //                            (*myCalibration)[detid.rawId()
                  //                            << 3 | APV]
                  //                            << std::endl;
                  if ((*myCalibration)[detid.rawId() << 3 | APV] != 0) {
                    //                    std::cout << "vorm strip gain setzen"
                    //                    << std::endl;
                    gain = (*myCalibration)[detid.rawId() << 3 | APV];
                    //                    std::cout << "nachm strip gain setzen"
                    //                    << std::endl;
                  } else
                    gain = 0;
                }
            ...
            if (charge > 0) {
              ...
              //              if (onlyOneAPV < 2)
              APVId_.push_back(APV);
              //              else
              //              APVId_.push_back(-1);
              stripRawId_.push_back(detid.rawId());

              harmonic2Strip +=
                  pow((scaleFactor * Norm * charge / dedxHits->pathlength(h)),
                      -2.0);
              stripCount++;
              stripSubDetId_.push_back(detid.subdetId());
              stripHitEta_.push_back(dedxHits->pos(h).eta());
              stripHitPhi_.push_back(dedxHits->pos(h).phi().phi());
            }
          } else if (detid.subdetId() < 3) {
            scaleFactor *= dEdxSF[1];
            const SiPixelCluster *cluster = dedxHits->pixelCluster(h);
            double gain = 1.0;
            charge = 0;
            int prevROC = -1;
            int ROC = -1;
            ...
            for (int l = 0; l < cluster->size(); l++) {
              ...
              if (cluster->pixel(l).x > 80)
                secondRow = true;
              if (secondRow) {
                if (cluster->pixel(l).y < 53) {
                  ROC = 8;
                } else if (cluster->pixel(l).y < 105) {
                  ROC = 9;
                } else if (cluster->pixel(l).y < 157) {
                  ROC = 10;
                } else if (cluster->pixel(l).y < 209) {
                  ROC = 11;
                } else if (cluster->pixel(l).y < 261) {
                  ROC = 12;
                } else if (cluster->pixel(l).y < 313) {
                  ROC = 13;
                } else if (cluster->pixel(l).y < 365) {
                  ROC = 14;
                } else if (cluster->pixel(l).y < 417) {
                  ROC = 15;
                }
              } else {
                if (cluster->pixel(l).y < 53) {
              ...
              if (ROC != prevROC) {
                if ((TrackerGains != NULL) && (useHSCPCalibration_)) {
                  gain = TrackerGains->at(detid.rawId() << 3 | ROC);
                }
                if ((myCalibration != NULL) && (useMyCalibration_)) {
                  //                  std::cout << "detid: " << detid.rawId() <<
                  //                  ", ROC: " << ROC
                  ...
                  //                            << 3 | ROC]
                  //                            << std::endl;
                  if ((*myCalibration)[detid.rawId() << 3 | ROC] != 0) {
                    //                    std::cout << "vorm pixel gain setzen"
                    //                    << std::endl;
                    gain = (*myCalibration)[detid.rawId() << 3 | ROC];
                    //                    std::cout << "nachm pixel gain setzen"
                    //                    << std::endl;
                  } else
                    gain = 0;
                }
              ...
              if (gain != 0)
                charge += (int)(cluster->pixelADC()[l] / gain);
              else
                charge = 0;
            }
            if (charge > 0) {
              dEdxPerPixel_.push_back(scaleFactor * Norm * charge /
                                      dedxHits->pathlength(h));
              //              if (onlyOneROC < 2)
              ROCId_.push_back(ROC);
              //              else ROCId_.push_back(-1);
              pixelRawId_.push_back(detid.rawId());

              harmonic2Pixel +=
                  pow((scaleFactor * Norm * charge / dedxHits->pathlength(h)),
                      -2.0);
              pixelCount++;
              pixelSubDetId_.push_back(detid.subdetId());
              pixelHitEta_.push_back(dedxHits->pos(h).eta());
              pixelHitPhi_.push_back(dedxHits->pos(h).phi().phi());
            }
          }
          if (charge > 0) {
            harmonic2 += pow(
...
void DemoAnalyzer::beginJob() {
  ...
  if (isData_ && useHSCPCalibration_) {
  ...
  if (isData_ && useMyCalibration_) {
    std::ifstream file("Demo/DemoAnalyzer/data/MyCalibration.txt");
    unsigned int key;
    double value;
    while (file >> key >> value) {
      (*myCalibration)[key] = value;
    }
...
void DemoAnalyzer::beginRun(edm::Run const &iRun,
  ...
  run_ = iRun.id().run();
  if (isData_ && useHSCPCalibration_) {
    std::map<unsigned int, std::unordered_map<unsigned int, double>>::iterator
        it,
    ...
    TrackerGains = &(itPrev->second); // just in case we go beyond the list
    //    of run for which we have a correction
  }
}
