...
DemoAnalyzer::DemoAnalyzer(const edm::ParameterSet &iConfig)
  ...
  edm::InputTag tag1("offlinePrimaryVertices");
  edm::InputTag tag2("generalTracks");
  edm::InputTag tag3("dedxHarmonic2");
  edm::InputTag tag4("genParticles");
  consumes<reco::VertexCollection>(tag1);
  consumes<reco::TrackCollection>(tag2);
  consumes<edm::ValueMap<reco::DeDxData>>(tag3);
  consumes<reco::GenParticleCollection>(tag4);
...
void DemoAnalyzer::analyze(const edm::Event &iEvent,
  ...
  for (unsigned int i = 0; i < tracks->size(); i++) {
    if ((std::abs(tracks->at(i).eta()) < etaCut_) &&
        ...
        (std::abs(tracks->at(i).dz(primaryVertices->at(0).position())) <
         dZCut_)) {
      nTracks_++;
      ...
      //        if
      (tracks->at(i).recHit(j)->detUnit()->type().isTrackerPixel())
          //        {
          //          std::cout << "Pixel" << std::endl;
          //        }
          //        if
          (tracks->at(i).recHit(j)->detUnit()->type().isTrackerStrip())
      //        {
