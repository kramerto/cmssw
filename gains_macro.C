...
void gains_macro() {
  ...
  TTree *tree_ = (TTree *)file_->Get("Gains_274094_to_274197");

  tree_->Draw("Index:APVId");
  TGraph *g = new TGraph(88624, tree_->GetV1(), tree_->GetV2());
  g->Draw("ap");
  g->GetYaxis()->SetRangeUser(-1, 16);
  ...
  for (unsigned int i = 0; i < run.size(); i++) {
    ...
    while (reader.Next()) {
      ...
      if (i == 0) {
        std::cout << "Index: " << IndexInt << ", DetId: " << DetIdInt
                  << ", DetId&APVId: " << (DetIdInt << 3 | APVChar)
                  << ", Gain: " << GainDouble << ", APVId: " << (int)APVChar
                  << std::endl;
      }
  ...
  for (unsigned int j = 0; j < 6; j++) {
    ...
    for (unsigned int i = 0; i < /*GainsVector.size() / run.size()*/ 88624;
         i++) {
